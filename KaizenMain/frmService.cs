...
namespace KaizenMain
{
    public partial class frmService : Form
    {
        ...
        private void tabService_SelectedIndexChanged(object sender, EventArgs e)
        {
            selectedTab = tabService.SelectedIndex;
            tabService.TabPages[tabService.SelectedIndex].Focus();
            tabService.TabPages[tabService.SelectedIndex].CausesValidation = true;

            if (dgvService.SelectedRows.Count == 0 && (tabService.SelectedIndex == 1 || tabService.SelectedIndex == 3 || tabService.SelectedIndex == 4))
                tabService.TabPages[tabService.SelectedIndex].CausesValidation = true;
            else
            {
                switch (tabService.SelectedIndex)
                {
                    case 0:
                        {
        ...
        private void frmService_Shown(object sender, EventArgs e)
        {
            tabService.TabPages[0].CausesValidation = true;
            tabService.TabPages[0].Validating += new CancelEventHandler(AddTabValidate);

            tabService.TabPages[2].CausesValidation = true;
            tabService.TabPages[2].Validating += new CancelEventHandler(EditTabValidate);
        }
        ...
        void AddTabValidate(object sender, CancelEventArgs e)
        {
            ...
            else if (dgvService.SelectedRows.Count == 1)
            {

                equipSelected = true;

                equipIDSelected = seperateNumber(dgvService.SelectedRows[0].Cells[0].Value.ToString());
            }
        }
        ...
        private void Service_Load(object sender, EventArgs e)
        {
            ...
            sqlEquip = @"select * from Trans where TransType = 'Service'";
            ...
            dgvService.DataSource = dsKaizen.Tables["Trans"];
            dgvService.AutoResizeColumns(DataGridViewAutoSizeColumnsMode.AllCells);
            dgvService.Columns["TransType"].Visible = false;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                drTrans = dsKaizen.Tables["Trans"].Rows.
                    Find(dgvService.SelectedRows[0].Cells[0].Value);
        ...
        {
            daTransD.Update(dsKaizen, "TransDetails");
            if (btnEditService.Text == "EDIT SERVICE")
            {
                enableEdittxtB();

                btnEditService.Text = "SAVE";
            }
            else
            {
